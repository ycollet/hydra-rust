...
fn try_open(index: u32, format_type: RequestedFormatType) -> Result<Camera, nokhwa::NokhwaError> {
    Camera::new(CameraIndex::Index(index), RequestedFormat::new::<RgbFormat>(format_type))
}
...
fn open_camera(slot: usize, index: u32, tx: &SyncSender<SlotMessage>) -> Option<Camera> {
    ...
    log::info!("slot {slot}: camera {index} ({name}) started at {}x{}", res.width(), res.height());
