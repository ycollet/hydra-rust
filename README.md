...
| `P5(...)` / `new P5(...)` | Returns a plain settable map (like `hydraText`) rather than hard-erroring, so an assignment (`let p1 = P5(...)`) and later property reads/writes on it still work — p5.js is a whole separate creative-coding framework with no Rust equivalent here. A handful of its most commonly-called instance methods (`hide`, `show`, `textSize`, `fill`, `stroke`, `strokeWeight`) are additionally registered as no-ops on that map; the rest of its (large) API isn't, and unregistered numeric properties (`p1.frameCount`, `mouseX`, ...) read as `()`, which arithmetic (`*`) treats as `0` rather than hard-failing |
